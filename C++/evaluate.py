...
def getTask():
	...
	for i in range(max_threads):
		boxes[i] = True
	while True:
		current_task = db.tasks.find_one()
		if current_task:
			try:
				db.tasks.delete_one(current_task)
				job = multiprocessing.Process(target=judge, args=(current_task['_id'], current_task['code'], current_task['user'], current_task['task'], current_task['lang'], current_task['contest'], current_task['contestName'], boxes))
				jobs.append(job)
				job.start()
